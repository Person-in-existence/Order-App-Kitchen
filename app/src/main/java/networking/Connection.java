...
class Connection extends Thread {
        ...
        while (open) {
            try {
                try {
                    ...
                    Log.v("networking.Connection", ip + ": packet handling complete");
                ...
                if (toSend != null) {
                    Log.v("networking.Connection", "sending packet: " + toSend);
                    toSend.send(out);
                ...
                if (triesReconnect) {
                    ...
                    if (!reconnectBackoff()) {
                        ...
                        Log.e("networking.Connection", "Reconnect failed - connection disconnected");
                        close(false);
                    } else {
                        Log.d("networking.Connection", "Reconnect success!");
                    }
                } else {
    ...
    private void handleType9(Header header) throws IOException {
        ...
        if (!receivedIdempotencies.contains(header.idempotencyToken)) {
            ...
            Log.w("networking.Connection", "Received duplicate idempotency - no action taken - from IP " + ip);
        }
        // Send a confirmation
